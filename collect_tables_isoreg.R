...
for (m in models){
    ...
    if (m==1|m==2) rate.adj <- rep(c(1,(subs/n)^(1/3),rep(1,n.outrow-5),n^(-1/3)),S)
    if (m==3) rate.adj <- rep(c(1,(subs/n)^(3/7),rep(1,n.outrow-5), n^(-3/7)),S)
...
output.N1 <- latex(round(out,3), file = "table_isoreg.txt",
                   ...
                   n.rgroup = c(1,3,1,n.outrow-5),
                   rgroup = c("Standard","m-out-of-n","Plug-in","Reshaped"),
